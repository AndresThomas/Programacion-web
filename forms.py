from django.contrib.auth.models import User
from django import forms

ERROR_MESSAGE_USER={'unique':'Este user name ya existe pruebe con otro','invalid':'usuario o contraseña incorrectos'}
ERROR_MESSAGE_PASSWORD={'required':'El password es requerido'}
ERROR_MESSAGE_EMAIL={'invalid':'Por favor ingrese un correo valido'}

class RegisterBussinessForm(forms.ModelForm):
    firs_name = forms.CharField(label='',required=True)
    last_name = forms.CharField(label='',required=True)
    password = forms.CharField(max_length=20,widget=forms.PasswordInput(),required=True,error_messages= ERROR_MESSAGE_PASSWORD)
    email = forms.CharField(required=True, error_messages=ERROR_MESSAGE_EMAIL)
    
    def __init__(self,*args,**kargs):
        super(RegisterBussinessForm,self).__init__(*args,**kargs)
        self.fields['username'].widget.attrs.update({'id':'username_create','class':'form-control','placeholder':'Usuario'})
        self.fields['first_name'].widget.attrs.update({'id':'first_name_create','class':'form-control','placeholder':'Nombre'})
        self.fields['last_name'].widget.attrs.update({'id':'last_name_create','class':'form-control','placeholder':'Apellidos'})
        self.fields['password'].widget.attrs.update({'id':'password_create','class':'form-control','placeholder':'Contraseña'})
        self.fields['email'].widget.attrs.update({'id':'email_create','class':'form-control','placeholder':'Email'})
    
    class Meta:
        model = User
        fields=['username','email','firs_name','last_name','password']

